...
def process_init():
    current_process.bcad = BikeCAD()

def init_mp(reserve_threads=1):
    thread_count = mp.cpu_count()
    print(f"CPU Thread Count: {thread_count}; reserving {reserve_threads}")
    global pool
    pool = mp.Pool(processes=mp.cpu_count()-reserve_threads, initializer = process_init)
    return thread_count-reserve_threads
...
def svg_auxilary_function(inputs):
    i,flist = inputs
    if len(flist)>0:
        current_process.bcad.export_svg_from_list(flist)
...
def svg_auxilary_function_sig(inputs):
    ...
    if len(flist)>0:
        ...
        try:
            ...
            pid = current_process.bcad.instance.pid
            print("Timeout in instance {pid}!")
...
def png_auxilary_function(inputs):
    i,flist = inputs
    if len(flist)>0:
        current_process.bcad.export_png_from_list(flist)
